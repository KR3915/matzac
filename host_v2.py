...
class GameState:
    def __init__(self):
        ...
        self.host_shot_x = None
        self.host_shot_y = None
        self.host_shot_active = False
        
        # Klient (nahoře)
        self.client_x = PLAYER_START_X
        self.client_y = CLIENT_START_Y
        self.client_alive = True
        self.client_shot_x = None
        self.client_shot_y = None
        self.client_shot_active = False
    ...
    def host_shoot(self):
        if not self.host_alive or self.host_shot_active:
            return
        self.host_shot_x = self.host_x
        self.host_shot_y = self.host_y - 1
        self.host_shot_active = True

    def client_shoot(self):
        if not self.client_alive or self.client_shot_active:
            return
        self.client_shot_x = self.client_x
        self.client_shot_y = self.client_y + 1
        self.client_shot_active = True

    def update_shots(self):
        # Pohyb střel hosta (nahoru)
        if self.host_shot_active:
            self.host_shot_y -= 1
            if self.host_shot_y < 0:
                self.host_shot_active = False
            else:
                # Kontrola zásahu nepřátel
                for enemy in self.enemies:
                    if (enemy['alive'] and 
                        enemy['x'] == self.host_shot_x and 
                        enemy['y'] == self.host_shot_y):
                        enemy['alive'] = False
                        self.host_shot_active = False
                        self.host_score += 10
                        break
                ...
                if (self.client_alive and 
                    self.client_x == self.host_shot_x and 
                    self.client_y == self.host_shot_y):
                    self.client_alive = False
                    self.host_shot_active = False

        # Pohyb střel klienta (dolů)
        if self.client_shot_active:
            self.client_shot_y += 1
            if self.client_shot_y >= DISPLAY_HEIGHT:
                self.client_shot_active = False
            else:
                # Kontrola zásahu nepřátel
                for enemy in self.enemies:
                    if (enemy['alive'] and 
                        enemy['x'] == self.client_shot_x and 
                        enemy['y'] == self.client_shot_y):
                        enemy['alive'] = False
                        self.client_shot_active = False
                ...
                if (self.host_alive and 
                    self.host_x == self.client_shot_x and 
                    self.host_y == self.client_shot_y):
    ...
    def to_dict(self):
        return {
            ...
            'host_shot_x': self.host_shot_x,
            'host_shot_y': self.host_shot_y,
            'host_shot_active': self.host_shot_active,
            'client_x': self.client_x,
            'client_y': self.client_y,
            'client_alive': self.client_alive,
            'client_shot_x': self.client_shot_x,
            'client_shot_y': self.client_shot_y,
            'client_shot_active': self.client_shot_active,
    ...
    def from_dict(self, data):
        ...
        self.host_shot_x = data.get('host_shot_x')
        self.host_shot_y = data.get('host_shot_y')
        self.host_shot_active = data.get('host_shot_active', False)
        ...
        self.client_shot_x = data.get('client_shot_x')
        self.client_shot_y = data.get('client_shot_y')
        self.client_shot_active = data.get('client_shot_active', False)
...
def render_display(game_state):
    ...
    # Vykreslíme střely
    if game_state.host_shot_active:
        display.set_pixel(game_state.host_shot_x, game_state.host_shot_y, "yellow")
    if game_state.client_shot_active:
        display.set_pixel(game_state.client_shot_x, game_state.client_shot_y, "yellow")
